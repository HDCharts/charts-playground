...
total_line_text() {
  ...
  if ((effective_broken == 0)); then
    if ((total_skipped > 0)); then
      ...
      printf -- '- ✅ %s %s completed successfully' "$total_tests" "$word"
    fi
    return
  fi

  if ((total_broken == 0 && gradle_broken == 1)); then
    printf -- '- ❌ Test workflow failed before complete test results were produced'
    return
  fi

  if ((total_skipped > 0)); then
    printf -- '- ❌ %s %s completed, %s failed (%s skipped)' "$total_tests" "$word" "$effective_broken" "$total_skipped"
  else
    printf -- '- ❌ %s %s completed, %s failed' "$total_tests" "$word" "$effective_broken"
  fi
}
...
main() {
  ...
  read -r playground_tests playground_failures playground_errors playground_skipped < <(
    collect_suite_counts "${PLAYGROUND_RESULT_DIRS[@]}"
  )

  local gradle_broken total_tests total_failures total_errors total_skipped summary_line
  total_tests=$((playground_tests))
  total_failures=$((playground_failures))
  total_errors=$((playground_errors))
  total_skipped=$((playground_skipped))
  gradle_broken="$(gradle_step_broken)"
  summary_line="$(total_line_text "$total_tests" "$total_failures" "$total_errors" "$total_skipped" "$gradle_broken")"
